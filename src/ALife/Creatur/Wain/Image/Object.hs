------------------------------------------------------------------------
-- |
-- Module      :  ALife.Creatur.Wain.Image.Object
...
import ALife.Creatur.Wain.Image.Pattern (Pattern)
import ALife.Creatur.Wain.Image.Wain (PatternWain)
...
objectId (PObject _ s) = "Image " ++ s
objectId (AObject a) = agentId a

objectNum :: Object a rt -> Int
objectNum (PObject _ s) = read [head s]
...
objectToWain (PObject _ _) = error "image, not wain"
